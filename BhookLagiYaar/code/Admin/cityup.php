...
if(isset($_REQUEST['Submit']))
{
	...
	$query = "insert into m_city (city_name,city_code,notes,status) values ('".mysql_real_escape_string(stripslashes($_POST['city_name']))."', '".mysql_real_escape_string(stripslashes($_POST['city_code']))."' , '".mysql_real_escape_string(stripslashes($_POST['city_description']))."', '".mysql_real_escape_string(stripslashes($_POST['city_status']))."') " ;	
...
{
	echo "in update";
	$query = "UPDATE m_city SET city_name = '".$_POST['city_name']."', city_code = '".$_POST['city_code']."', notes = '".$_POST['city_description']."', status = '".$_POST['city_status']."' WHERE city_id = '".$_REQUEST['cid']."'";
...
?>
                ...
                if($_REQUEST['cid'])
				{
				$query1 = "SELECT * FROM m_city WHERE city_id= '".$_REQUEST['cid']."'";
		...
		$city_id = $rows1['city_id'];	
							...
							<form class="form uniformForm validateForm" method="post" action="" enctype="multipart/form-data" name="news">
								<div class="field-group">
									<label for="required">City Name </label><span class="field">
									  <input type="text" id="title1" name="city_name" class="validate[required] cols="50" " value="<?php echo $rows1['city_name']?>">
									</span> 
									<div class="field"></div>
								</div>
                                <div class="field-group">
									<label for="required">City Code </label><span class="field">
									  <input type="text" id="title2" name="city_code" class="validate[required] cols="50" " value="<?php echo $rows1['city_code']?>">
									</span> 
									<div class="field"></div>
								</div>
								<div class="field-group">
									...
									<div class="field">
										<textarea name="city_description" id="title" rows="5" cols="50"> <?php echo $rows1['notes']?></textarea>	
									</div>
								</div> <!-- .field-group -->
            ...
            </div><input type="hidden" value="<?php echo $_REQUEST['cid'];?>" name="cid" />
				...
				?>
					...
					<div class="widget-content">
							...
							<form class="form uniformForm validateForm" method="post" action="" enctype="multipart/form-data" name="news">
								<div class="field-group">
									<label for="required">City Name </label><span class="field">
									  <input type="text" id="title1" name="city_name" class="validate[required] cols="50" " >
									</span> 
									<div class="field"></div>
								</div>
                                <div class="field-group">
									<label for="required">City Code </label><span class="field">
									  <input type="text" id="title2" name="city_code" class="validate[required] cols="50" ">
									</span> 
									<div class="field"></div>
								</div>
								<div class="field-group">
